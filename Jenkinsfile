            ...
            steps{
                script {
                    withCredentials([usernamePassword(credentialsId: 'portainer-creds', passwordVariable: 'password', usernameVariable: 'username')]) {
                        ...
                        sh "curl -X POST 'http://portainer:9000/api/endpoints/1/docker/services/create' -H 'accept: application/json' -H 'Authorization: Bearer $jwt' -d '{\"Name\": \"my-service\", \"TaskTemplate\": {\"ContainerSpec\": {\"Image\": \"my-docker-image\"}}, \"Networks\": [{\"Target\": \"my-network\"}]}'"
